...
function createFutureFeaturesSheet() {
  ...
  if (!sheet) {
    sheet = ss.insertSheet(SHEETS.FUTURE_FEATURES);
  }
  sheet.clear();
  ...
  const headers = ["Feature", "Description", "Priority", "Complexity",
                   "Estimated Timeline", "Requested By", "Status"];
...
function createPendingFeaturesSheet() {
  ...
  if (!sheet) {
    sheet = ss.insertSheet(SHEETS.PENDING_FEATURES);
  }
  sheet.clear();

  sheet.getRange("A1:H1").merge()
    .setValue("⏳ PENDING FEATURES")
    .setFontSize(16)
    .setFontWeight("bold")
    .setHorizontalAlignment("center")
    .setBackground(COLORS.ACCENT_YELLOW)
    .setFontColor(COLORS.TEXT_DARK);

  const headers = ["Feature", "Description", "Start Date", "Expected Completion",
                   "Progress %", "Blockers", "Assigned To", "Notes"];
